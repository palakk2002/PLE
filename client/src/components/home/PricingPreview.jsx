          ...
          {pricing.map((plan, index) => (
            <motion.div
              ...
              className={`relative rounded-2xl overflow-hidden transition-all duration-500 flex flex-col ${
                plan.highlight 
                  ? 'border-primary shadow-[0_0_25px_rgba(215,25,32,0.15)] bg-app-card scale-100 lg:scale-105 z-10' 
                  : 'border-app-border hover:border-primary/50 bg-app-card'
              } border backdrop-blur-xl group`}
            >
              {/* Highlight Glow */}
              <div 
                className="absolute inset-0 bg-gradient-to-b from-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
              />

              {plan.highlight && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 bg-primary text-black text-[10px] font-bold uppercase tracking-widest px-4 py-1 rounded-b-lg shadow-[0_4px_10px_rgba(215,25,32,0.3)] z-20">
                  Most Popular
                </div>
              )}
                ...
                <ul className="space-y-3 mb-8 flex-grow">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <div className={`mt-1 rounded-full p-0.5 shrink-0 ${plan.highlight ? 'bg-primary/20 text-primary' : 'bg-app-bg border border-app-border text-app-text-muted'}`}>
