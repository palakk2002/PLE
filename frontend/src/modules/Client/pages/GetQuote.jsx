...
const serviceFormConfig = {
  ...
  performance_marketing: {
    ...
    radios: [
      ...
      { name: 'pmPrimaryGoal', label: 'client-primary Goal', options: ['LEADS', 'SALES', 'AWARENESS', 'APP INSTALLS'] },
      { name: 'projectBudget', label: 'Project Budget (INR)', options: ['BELOW 25K', '25K-75K', '75K-2L', '2L-5L', 'ABOVE 5L', 'NOT SURE'] }
    ],
    inputs: [
...
export default function GetQuote() {
      ...
      {[1, 2, 3, 4].map((s) => (
        <div key={s} className="flex items-center">
          <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-medium transition-colors duration-300 ${
            step === s ? 'bg-client-primary text-black shadow-[0_0_15px_rgba(215,25,32,0.4)]' : 
            step > s ? 'bg-client-primary/20 text-client-primary border border-client-primary/30' : 
          ...
          {s < 4 && (
            <div className={`w-8 sm:w-16 h-1 rounded-full mx-1 sm:mx-2 transition-colors duration-300 ${
              step > s ? 'bg-client-primary/50' : 'bg-app-border/30'
            }`} />
          )}
        </div>
      ))}
  ...
  const renderDynamicFields = () => {
    ...
    return (
      <div className="space-y-4">
        <div className="mb-4">
          <h3 className="text-xl font-black font-heading text-client-primary uppercase">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {config.dropdowns.map((dd, idx) => (
              <div key={idx} className="space-y-1.5 text-left relative group">
                ...
                <select
                  className={`w-full bg-transparent border-b-2 py-2.5 text-app-text focus:outline-none focus:border-client-primary transition-colors text-sm font-bold appearance-none ${
              ...
              {config.checkboxes.options.map(opt => (
                <label key={opt} className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${
                    (checkboxState[config.checkboxes.name] || []).includes(opt) ? 'bg-client-primary border-client-primary' : 'border-app-border/70 group-hover:border-client-primary/50'
                  }`}>
                  ...
                  {r.options.map(opt => (
                    <label key={opt} className="flex items-center cursor-pointer group">
                      ...
                      <div className={`px-3 py-1.5 rounded-full border flex items-center justify-center transition-colors text-[10px] font-extrabold tracking-wider ${
                        watch(r.name) === opt ? 'border-app-text text-app-text bg-app-border/20' : 'border-app-border/70 text-app-text-muted group-hover:border-client-primary/50 group-hover:text-app-text'
                      }`}>
                        {opt}
                      </div>
                    </label>
            ...
            {config.inputs.map((inp, idx) => (
              <div key={idx} className="space-y-1 text-left relative group">
                ...
                <input
                  ...
                  className={`w-full bg-transparent border-b-2 py-1.5 text-app-text focus:outline-none focus:border-client-primary transition-colors text-sm font-bold ${
        ...
        {config.textarea && (
          <div className="space-y-1.5 text-left relative group">
            <label className="text-[10px] font-extrabold text-app-text-muted/70 uppercase tracking-widest block transition-colors group-focus-within:text-client-primary">
              {config.textarea.label}
            </label>
            <textarea
              rows="3"
              placeholder={config.textarea.placeholder || "Tell us about your requirements..."}
              className={`w-full bg-transparent border-b-2 py-2.5 text-app-text focus:outline-none focus:border-client-primary transition-all resize-none text-sm font-bold placeholder:text-app-text-muted/20 ${
  ...
  return (
    <div className="min-h-screen bg-app-bg text-app-text relative pt-36 pb-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      {/* Decorative Orbs */}
      <div className="absolute -top-20 -left-20 w-[450px] h-[450px] bg-client-primary/5 rounded-full filter blur-[150px] pointer-events-none z-0" />
      <div className="absolute -bottom-40 right-20 w-96 h-96 bg-client-primary/4 rounded-full filter blur-[150px] pointer-events-none z-0" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-start">
            ...
            <AnimatePresence mode="wait">
              {!isSubmitSuccess ? (
                  ...
                  <form onSubmit={handleSubmit(onSubmitForm)} className="space-y-6">
                    ...
                    {step === 1 && (
                      <motion.div key="step1" variants={scrollFadeUp} initial="hidden" animate="visible" exit={{ opacity: 0, x: -20 }} className="space-y-4">
                        <h3 className="text-xl font-black font-heading text-client-primary mb-3">Contact Information</h3>
                        
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="space-y-1 text-left relative group">
                            <label className="text-[10px] font-extrabold text-app-text-muted/70 uppercase tracking-widest block">
                              First Name <span className="text-client-primary">*</span>
                            </label>
                            <input
                              type="text"
                              placeholder="e.g. Rajesh"
                              className={`w-full bg-transparent border-b-2 py-1.5 text-app-text focus:outline-none focus:border-client-primary transition-colors text-sm font-bold ${errors.firstName ? 'border-red-500/50' : 'border-app-border/70'}`}
                          ...
                          <div className="space-y-1 text-left relative group">
                            ...
                            <input
                              ...
                              className="w-full bg-transparent border-b-2 py-1.5 text-app-text focus:outline-none focus:border-client-primary transition-colors text-sm font-bold border-app-border/70"
                              {...register('lastName')}
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="space-y-1 text-left relative group">
                            <label className="text-[10px] font-extrabold text-app-text-muted/70 uppercase tracking-widest block">
                              Email Address <span className="text-client-primary">*</span>
                            </label>
                            <input
                              type="email"
                              placeholder="you@company.com"
                              className={`w-full bg-transparent border-b-2 py-1.5 text-app-text focus:outline-none focus:border-client-primary transition-colors text-sm font-bold ${errors.email ? 'border-red-500/50' : 'border-app-border/70'}`}
                          ...
                          <div className="space-y-1 text-left relative group">
                            <label className="text-[10px] font-extrabold text-app-text-muted/70 uppercase tracking-widest block">
                              Phone Number <span className="text-client-primary">*</span>
                            </label>
                            <input
                              type="tel"
                              placeholder="+91 98765 43210"
                              className={`w-full bg-transparent border-b-2 py-1.5 text-app-text focus:outline-none focus:border-client-primary transition-colors text-sm font-bold ${errors.phone ? 'border-red-500/50' : 'border-app-border/70'}`}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="space-y-1 text-left relative group">
                            ...
                            <select
                              className="w-full bg-transparent border-b-2 py-1.5 text-app-text focus:outline-none focus:border-client-primary transition-colors text-sm font-bold border-app-border/70 appearance-none"
                              {...register('city')}
                            >
                    ...
                    {step === 2 && (
                      <motion.div key="step2" variants={scrollFadeUp} initial="hidden" animate="visible" exit={{ opacity: 0, x: -20 }} className="space-y-6">
                        <div className="mb-4">
                          <h3 className="text-xl font-black font-heading text-client-primary mb-2">Select Your Service</h3>
                            ...
                            return (
                              <div 
                                ...
                                className={`p-4 rounded-xl border-2 cursor-pointer transition-all duration-300 flex items-start gap-4 ${
                                  isSelected 
                                    ? 'border-client-primary bg-client-primary/10 shadow-[0_4px_20px_rgba(215,25,32,0.15)]' 
                                    : 'border-app-border/50 bg-app-bg/50 hover:border-client-primary/50'
                                }`}
                              >
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${isSelected ? 'bg-client-primary text-black' : 'bg-app-border/20 text-app-text-muted'}`}>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {timelineOptions.map(opt => (
                              <label key={opt} className={`cursor-pointer text-center p-3 rounded-lg border transition-all text-sm font-bold ${
                                watch('timeline') === opt ? 'border-client-primary bg-client-primary/10 text-client-primary' : 'border-app-border/50 text-app-text-muted hover:border-client-primary/50'
                              }`}>
                    ...
                    {step === 4 && (
                      <motion.div key="step4" variants={scrollFadeUp} initial="hidden" animate="visible" exit={{ opacity: 0, x: -20 }} className="space-y-5">
                        ...
                        <div className="bg-app-bg border border-app-border/50 rounded-xl p-5 space-y-5 text-sm">
                          <div>
                            <h4 className="text-[10px] font-extrabold text-client-primary uppercase tracking-widest mb-2">Contact Info</h4>
                          ...
                          <div>
                            <h4 className="text-[10px] font-extrabold text-client-primary uppercase tracking-widest mb-2">Project Summary</h4>
                            <div className="grid grid-cols-2 gap-2 text-app-text-muted">
                          ...
                          <div>
                            <h4 className="text-[10px] font-extrabold text-client-primary uppercase tracking-widest mb-2">Details</h4>
                    ...
                    <div className="pt-6 flex items-center justify-between border-t border-app-border/30 mt-6">
                      {step > 1 ? (
                        <button
                          type="button"
                          onClick={prevStep}
                          className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-widest text-app-text-muted hover:text-client-primary transition-colors"
                        >
                ...
                <motion.div
                  ...
                  className="glass-panel rounded-3xl p-8 md:p-10 text-center space-y-6 flex flex-col items-center border border-client-primary/20 bg-app-card/85 shadow-2xl relative overflow-hidden"
                >
                  <div className="w-16 h-16 rounded-full bg-client-primary/10 border border-client-primary/20 text-client-primary flex items-center justify-center shadow-[0_0_35px_rgba(215,25,32,0.25)]">
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-client-primary/10 hover:bg-client-primary/20 border border-client-primary/30 text-client-primary hover:text-white rounded-full text-xs font-extrabold uppercase tracking-widest transition-all flex items-center gap-1.5 cursor-pointer"
                  >
          ...
          <div className="lg:col-span-5 relative flex items-center justify-center min-h-[500px] w-full z-10 select-none hidden lg:flex">
            ...
            >
              ...
              <motion.circle 
                cx="150" cy="110" r="22" 
                className="fill-client-primary/65 dark:fill-client-primary/45" 
                animate={{ y: [0, -12, 0] }}
            ...
            >
              <div className="space-y-1 mb-8 text-left">
                <div className="w-12 h-[3px] bg-client-primary rounded-full mb-3" />
              ...
              <div className="space-y-6 text-left w-full">
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    <a href={`tel:${CONTACT_INFO.phone}`} className="text-sm font-black text-zinc-950 dark:text-white hover:text-client-primary transition-colors font-sans">
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    <a href={`mailto:${CONTACT_INFO.email}`} className="text-sm font-black text-zinc-950 dark:text-white hover:text-client-primary transition-colors font-sans">
                      {CONTACT_INFO.email}
                    </a>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-3.5 mt-10">
                {[
                  { icon: FacebookIcon, href: 'https://www.facebook.com/share/1EaNrat2yr/' },
                  { icon: InstagramIcon, href: 'https://www.instagram.com/peoplesleagueofelectronics?igsh=MWdtbTNzajdqMGV4cQ==' },
                  { icon: TwitterIcon, href: 'https://x.com/PeoplesE9405' },
                  { icon: LinkedinIcon, href: 'https://www.linkedin.com/company/ple-electronics' },
                  { icon: YoutubeIcon, href: 'https://www.youtube.com/@ple' }
                ].map((social, idx) => (
                  <a 
                    key={idx} href={social.href} target="_blank" rel="noopener noreferrer"
                    className={`w-10 h-10 rounded-full bg-zinc-100 hover:bg-client-primary border border-zinc-200 hover:border-client-primary text-zinc-700 hover:text-black flex items-center justify-center shadow-md transition-all duration-300 hover:-translate-y-1 dark:bg-zinc-900 dark:border-zinc-800 dark:text-zinc-300 dark:hover:bg-client-primary dark:hover:text-black ${idx === 0 ? 'ml-3.5' : ''}`}
                  >
