...
const getCategoryIcon = (category) => {
  const iconClasses = "w-5 h-5 text-client-primary";
  switch (category) {
    case 'web':
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-[2.25rem] border-[1.5px] overflow-hidden flex flex-col justify-between h-full group transition-all duration-500
        ${highlight 
          ? 'border-client-primary bg-white dark:bg-app-card shadow-[0_0_30px_rgba(215,25,32,0.15)] dark:shadow-[0_0_30px_rgba(215,25,32,0.25)]' 
          : 'border-slate-100 dark:border-white/10 bg-white dark:bg-app-card hover:border-client-primary hover:shadow-[0_0_30px_rgba(215,25,32,0.2)] shadow-sm dark:shadow-lg'
        }`}
    >
      {/* Floating best choice badge if highlighted */}
      {highlight && (
        <div className="absolute top-2.5 right-3.5 z-20">
          <span className="px-3 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest bg-client-primary text-white dark:bg-white dark:text-black shadow-md">
            Best Choice
          </span>
        </div>
      )}
      ...
      <div className="flex flex-col flex-grow">
        ...
        <div className="flex items-center gap-4 p-6 pt-7">
          <div className="w-12 h-12 rounded-2xl bg-client-primary/10 border border-client-primary/25 flex items-center justify-center shrink-0 shadow-sm transition-transform duration-500 group-hover:scale-110 group-hover:rotate-[360deg]">
            {getCategoryIcon(category)}
          </div>
          <div className="space-y-0.5 pr-12"> {/* Right padding to avoid overlap with badge */}
            <h3 className="text-base md:text-lg font-black font-heading text-black dark:text-white uppercase tracking-wide leading-tight transition-colors duration-300 group-hover:text-client-primary">
              {title}
            </h3>
            <p className="text-client-primary text-[9px] uppercase font-bold tracking-widest block">
              {tech}
            </p>
          </div>
        </div>
            ...
            <ul className="space-y-2.5 h-[200px] overflow-y-auto pr-2">
              {features.map((feature, idx) => (
                <li key={idx} className="flex items-start gap-2.5 group/item">
                  <div className="mt-1 w-3.5 h-3.5 rounded-full bg-client-primary/10 dark:bg-client-primary/20 flex items-center justify-center flex-shrink-0 group-hover:scale-115 transition-all">
                    <Check className="w-2 h-2 text-client-primary" strokeWidth={3} />
                  </div>
      ...
      <div className="bg-[#FFF6F2] dark:bg-[#07070F] p-6 border-t border-black/5 dark:border-white/5 rounded-b-[2.25rem] flex items-center justify-between gap-4 transition-colors duration-300">
        <div>
          ...
          <span className="text-[8px] font-black text-client-primary tracking-widest uppercase block mt-1">
            {periodLabel}
          </span>
        </div>
        ...
        <Link
          to="/get-quote"
          className="px-5 py-3 rounded-xl flex items-center justify-center gap-2 font-black text-[10px] uppercase tracking-wider bg-client-primary text-black hover:bg-black hover:text-client-primary dark:hover:bg-white dark:hover:text-black transition-all duration-300 shadow-[0_4px_12px_rgba(215,25,32,0.15)] hover:shadow-[0_6px_20px_rgba(215,25,32,0.35)] hover:-translate-y-0.5 active:translate-y-0"
        >
