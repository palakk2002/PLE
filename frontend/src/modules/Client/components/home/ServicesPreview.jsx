...
export default function ServicesPreview() {
  ...
  return (
    <section className="pt-8 sm:pt-12 pb-2 sm:pb-6 px-4 sm:px-6 lg:px-8 bg-app-bg relative overflow-hidden">
      ...
      <div className="absolute top-1/4 left-0 w-96 h-96 bg-client-primary/2 dark:bg-white/1 rounded-full filter blur-[130px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <motion.h2 variants={scrollFadeUp} className="text-3xl sm:text-5xl md:text-6xl font-black font-heading text-app-text leading-tight">
            {headerData.heading} <span className="text-client-primary text-gradient-orange">{headerData.highlightText}</span>
          </motion.h2>
        ...
        >
          {services && services.map((srv) => (
            <motion.div key={srv.id} variants={scrollFadeUp}>
              <ServiceCard 
                service={{ ...srv, iconName: srv.icon }} 
                image={srv.imageUrl || serviceImages[srv.id] || webDevImg}
              />
            </motion.div>
          ))}
        </motion.div>
        ...
        >
          ...
          >
            ...
            <span className="w-5 h-5 rounded-full bg-client-primary flex items-center justify-center group-hover:scale-110 transition-transform">
