...
/**
 * WhyChooseUs - Updated with useCMS hook.
 */
export default function WhyChooseUs() {
  ...
  useEffect(() => {
    if (!isMobile || !features || features.length === 0) return;
  ...
  const CardContent = ({ feature }) => {
    ...
    return (
      <>
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-client-primary/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl" />

        {/* Icon Wrapper */}
        <div className="mb-6 w-16 h-16 rounded-2xl bg-client-primary/10 dark:bg-white/5 flex items-center justify-center text-client-primary shadow-inner group-hover:bg-client-primary group-hover:text-black transition-all duration-500 transform group-hover:rotate-3">
          <Icon size={28} strokeWidth={2.5} />
        </div>

        {/* Title */}
        <h3 className="text-xl font-black font-heading text-client-primary mb-3 transition-colors duration-300 relative z-10">
          {feature.title}
        </h3>
        ...
        {/* Bottom Accent Line */}
        <div className="absolute bottom-0 left-0 w-0 h-1 bg-client-primary group-hover:w-full transition-all duration-700 rounded-b-2xl" />
      </>
    );
  };
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center mb-12 sm:mb-16 md:mb-20 space-y-4">
          <motion.span 
            ...
            viewport={{ once: true }}
            className="text-client-primary text-xs font-black uppercase tracking-[0.3em] bg-client-primary/10 px-4 py-1.5 rounded-full"
          >
            {header.tagline}
          </motion.span>
          <motion.h2 
          ...
          >
            {header.heading} <span className="text-client-primary">{header.highlightText}</span>
          </motion.h2>
          <motion.div 
            initial={{ scaleX: 0 }}
            ...
            className="w-24 h-1 bg-client-primary mx-auto rounded-full"
          />
        </div>
          ...
          >
            {features && features.map((feature, index) => (
              <motion.div
                ...
                className="group relative bg-white dark:bg-app-card rounded-2xl p-8 shadow-sm dark:shadow-lg border border-slate-100 dark:border-app-border hover:border-client-primary hover:shadow-xl dark:hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col items-start text-left"
              >
                <CardContent feature={feature} />
              </motion.div>
            ))}
