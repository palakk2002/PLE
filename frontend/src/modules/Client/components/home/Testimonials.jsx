...
export default function Testimonials() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-client-primary/5 rounded-full blur-[150px] pointer-events-none" />
      ...
      <div className="max-w-5xl mx-auto px-4 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 md:mb-10 gap-4">
          ...
          <div className="flex-shrink-0 flex justify-center md:justify-end">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-client-primary/10 hover:bg-client-primary/20 text-client-primary border border-client-primary/20 hover:border-client-primary/40 rounded-full font-semibold transition-all duration-300"
            >
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="relative bg-white dark:bg-app-card backdrop-blur-2xl rounded-[2rem] md:rounded-[2.5rem] rounded-tr-[3rem] md:rounded-tr-[4rem] p-5 md:p-8 shadow-[0_8px_30px_rgba(0,0,0,0.06)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.4)] text-left border border-slate-100 dark:border-white/10">
                ...
                <div className="absolute top-0 right-0 w-2/3 md:w-1/2 h-full border-t-[4px] md:border-t-[6px] border-r-[4px] md:border-r-[6px] border-client-primary opacity-80 rounded-tr-[3rem] md:rounded-tr-[4rem] rounded-br-[2.5rem] pointer-events-none" />
                
                {/* Top Left Profile Image Overlapping */}
                <div className="absolute -top-12 left-8 md:-top-16 md:-left-4 w-24 h-24 md:w-32 md:h-32 rounded-full border-[4px] border-client-primary overflow-hidden bg-white/80 dark:bg-black/50 backdrop-blur-xl shadow-xl z-20">
                  <img
                    src={t.avatar}
                    alt={t.author}
                  ...
                  <div className="flex justify-between items-center mb-3 pr-2">
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map((_, i) => (
                        ...
                        >
                          <Star className="w-4 h-4 md:w-5 md:h-5 fill-client-primary text-client-primary" />
                        </motion.div>
                      ))}
                    </div>
                    <Quote className="w-8 h-8 md:w-10 md:h-10 text-client-primary opacity-90 fill-client-primary rotate-180" />
                  </div>
        ...
        <div className="mt-4 md:mt-8 flex items-center justify-center gap-6 relative z-20">
          <button 
            onClick={prev} 
            className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-app-bg border border-app-border flex items-center justify-center text-app-text hover:bg-client-primary hover:text-black hover:border-client-primary transition-all duration-300 shadow-md"
          >
            ...
            {testimonials.map((_, i) => (
              <button
                ...
                className={`w-2 h-2 md:w-2.5 md:h-2.5 rounded-full transition-all duration-500 ${
                  i === index ? 'w-8 md:w-10 bg-client-primary opacity-80' : 'bg-slate-300 dark:bg-white/20 hover:bg-slate-400 dark:hover:bg-white/40'
                }`}
