      ...
      <div className="w-full bg-app-bg border-b border-app-border text-app-text-muted py-2 px-4 md:px-8 z-50 relative hidden sm:block">
        <div className="max-w-6xl mx-auto flex justify-between items-center text-[11px] font-medium tracking-wide">
          <div className="flex gap-6">
            <a href={`tel:${contact.phone}`} className="flex items-center gap-1.5 hover:text-client-primary transition-colors cursor-pointer">
              <Phone className="w-3.5 h-3.5" /> {contact.phoneDisplay}
            </a>
            <a href={`mailto:${contact.email}`} className="flex items-center gap-1.5 hover:text-client-primary transition-colors cursor-pointer">
      ...
      >
        <div className={`w-full flex justify-between items-center pointer-events-auto transition-all duration-500 px-4 py-0 sm:py-0 rounded-full shadow-xl ${
          isScrolled 
            ...
            : 'md:bg-transparent md:border-transparent md:shadow-none bg-white/70 dark:bg-[#1A1A2E]/30 backdrop-blur-md border border-slate-200/50 dark:border-client-primary/10'
        }`}>
            ...
            <nav 
              className={`flex items-center justify-center gap-1 h-7 px-1.5 rounded-full border shadow-2xl transition-all duration-500 ${
                isScrolled 
                  ...
                  : 'bg-white/50 dark:bg-[#1A1A2E]/30 backdrop-blur-md border-slate-200/50 dark:border-client-primary/10'
              }`}
            >
              {navLinks.map((link) => {
                      ...
                      {({ isActive }) => (
                        <>
                          ...
                          {isActive && (
                            <motion.div
                              layoutId="activePill"
                              className="absolute inset-0 bg-client-primary rounded-full shadow-[0_0_15px_rgba(215,25,32,0.4)] z-0"
                          ...
                          {!isActive && (
                            <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-client-primary transition-all duration-300 group-hover/nav:w-1/2 rounded-full" />
                          )}
                        </>
                      )}
                    </NavLink>
                          ...
                          {SERVICES.map((srv) => {
                            ...
                            return (
                              <Link
                                ...
                                className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-black/5 dark:hover:bg-client-primary/10 text-app-text-muted hover:text-app-text dark:hover:text-client-primary transition-all duration-200 group/dropitem cursor-pointer"
                              >
                                <div className="w-8 h-8 rounded-lg bg-client-primary/10 dark:bg-client-primary/5 border border-client-primary/20 dark:border-client-primary/10 flex items-center justify-center text-client-primary group-hover/dropitem:scale-110 transition-transform shrink-0">
                                ...
                                <div className="flex flex-col text-left overflow-hidden">
                                  <span className="text-xs font-bold text-app-text group-hover/dropitem:text-client-primary transition-colors truncate">{srv.title}</span>
          ...
          <div className="md:hidden flex items-center gap-3">
            ...
            <button
              ...
              className="p-2.5 rounded-full bg-[#1A1A2E] border border-white/10 text-white/70 hover:text-client-primary transition-all shadow-xl"
              ...
              <div className="p-3 space-y-1.5">
                {navLinks.map((link) => {
                  ...
                  return (
                    <div key={link.path} className="space-y-1">
                      {isServices ? (
                        <div className={`flex items-center justify-between px-5 py-3.5 rounded-2xl text-sm font-bold transition-all border border-transparent hover:border-client-primary ${
                          location.pathname === link.path
                            ? 'bg-client-primary text-black'
                          ...
                          <button
                            ...
                            className="p-1 rounded-full bg-slate-100 dark:bg-white/5 hover:text-client-primary transition-colors"
                          >
                        ...
                        <NavLink
                          ...
                          onClick={() => setIsMobileMenuOpen(false)}
                          className={({ isActive }) =>
                            `flex items-center justify-between px-5 py-3.5 rounded-2xl text-sm font-bold transition-all border border-transparent hover:border-client-primary ${
                              isActive
                                ? 'bg-client-primary text-black'
                                : 'text-slate-800 hover:bg-slate-100 hover:text-black dark:text-white/70 dark:hover:bg-white/5 dark:hover:text-white'
                            }`
                      ...
                      <AnimatePresence>
                        {isServices && isServicesExpanded && (
                          <motion.div 
                            ...
                            className="pl-4 pr-2 py-1 space-y-1 border-l-2 border-client-primary/20 ml-4 overflow-hidden"
                          >
                            {SERVICES.map((srv) => (
                              <Link
                                key={srv.slug}
                                to={`/services/${srv.slug}`}
                                onClick={() => setIsMobileMenuOpen(false)}
                                className="flex items-center justify-between px-4 py-2.5 rounded-xl text-xs font-medium text-slate-700 dark:text-white/70 hover:bg-client-primary/10 hover:text-client-primary transition-all border border-transparent hover:border-client-primary"
                              >
                                <span>{srv.title}</span>
                ...
                <div className="pt-2">
                  <Link
                    to="/get-quote"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="w-full py-4 rounded-2xl font-black text-center bg-client-primary text-black flex items-center justify-center gap-2"
                  >
