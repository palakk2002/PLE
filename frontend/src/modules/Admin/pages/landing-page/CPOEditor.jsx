...
const CPOEditor = () => {
  ...
  const { cpoSection, updateCpoSection } = useLandingPageStore();

  const [formData, setFormData] = useState({
    title: cpoSection?.title || '',
    subtitle: cpoSection?.subtitle || '',
    description: cpoSection?.description || '',
    image: cpoSection?.image || '',
    features: cpoSection?.features || [],
    ctaText: cpoSection?.ctaText || '',
    ctaLink: cpoSection?.ctaLink || '',
    status: cpoSection?.status !== undefined ? cpoSection?.status : true,
  });
  ...
  const handleSave = () => {
    ...
    updateCpoSection(formData);
    toast.success('CPO section updated successfully!');
    ...
    >
      <div className="flex items-center justify-between border-b border-gray-100 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Certified Pre-Owned (CPO) Settings</h1>
            <p className="text-xs text-gray-500 mt-0.5">Manage CPO titles, description, features, image, and status.</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-4 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            ...
            {formData.image && (
              <img src={formData.image} alt="CPO Section" className="mt-3 max-h-40 rounded-lg border border-gray-100 object-cover" />
            )}
          </div>

          <div className="md:col-span-2 border-t border-gray-100 pt-4">
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">CPO Features</label>
            <div className="flex gap-2 mb-3">
              <input
                type="text"
                value={featureInput}
                onChange={(e) => setFeatureInput(e.target.value)}
                placeholder="e.g. 40-Point Inspection Checked"
                className="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#C07A3D]"
              />
              <button
                type="button"
