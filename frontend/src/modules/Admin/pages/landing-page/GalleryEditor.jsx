...
import { FiSave, FiArrowLeft, FiPlus, FiTrash, FiEdit, FiImage } from 'react-icons/fi';
...
const GalleryEditor = () => {
  ...
  const handleStartAdd = () => {
    ...
    setFormData({
      id: `gal-${Date.now()}`,
      url: 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?q=80&w=400&h=300&fit=crop',
      title: ''
    });
  };
  ...
  const handleSaveItem = () => {
    ...
    updateGallery(updatedList);
    setEditingIndex(null);
    toast.success('Gallery list updated successfully!');
  };

  const handleDeleteItem = (index) => {
    if (window.confirm('Delete this image from the gallery?')) {
      const updatedList = gallery.filter((_, idx) => idx !== index);
      updateGallery(updatedList);
    ...
    >
      <div className="flex items-center justify-between border-b border-gray-100 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Gallery Editor</h1>
            <p className="text-xs text-gray-500 mt-0.5">Add, edit, or remove images featured in the landing page gallery section.</p>
          </div>
        </div>
      ...
      {editingIndex !== null ? (
        <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-4 shadow-sm">
          <h2 className="text-sm font-bold text-[#C07A3D] mb-4 uppercase">
            {editingIndex === -1 ? 'Add Gallery Image' : 'Edit Gallery Image'}
          </h2>
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Image Title / Description</label>
            <input
              type="text"
              name="title"
          ...
          <div className="mt-4">
            <span className="block text-xs font-bold text-gray-500 uppercase mb-1">Image Preview</span>
            <img 
              src={formData.url} 
              alt="Preview" 
              className="max-h-48 rounded-lg object-cover border border-gray-200 mt-1"
              onError={(e) => {
                e.target.src = 'https://images.unsplash.com/photo-1594322436404-5a0526db4d13?q=80&w=400&h=300&fit=crop';
              }}
            />
          </div>
          ...
          {gallery.map((item, idx) => (
            <div key={item.id || idx} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition">
              ...
              <div className="p-4 flex-1 flex flex-col justify-between">
                <div>
                  <h3 className="text-sm font-bold text-gray-800 line-clamp-1">{item.title || 'Untitled Image'}</h3>
                </div>
