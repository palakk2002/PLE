...
## 2026-06-22 (Session 3 — B2B Employee Reload State Persistence & Self-Healing Sync)

### Updated

* **frontend/src/modules/UserApp/hooks/useBusinessBuyer.js**
  - Updated the `useBusinessBuyer` hook to dynamically check the user's role from the main `useAuthStore` session. If the user's authentication role is B2B Employee (`b2bEmployee`), B2B Admin (`b2bAdmin`), or employee (`isEmployee`), they are always treated as a B2B business buyer (`isBusiness = true`). This prevents the application from incorrectly fallback-rendering B2C layouts for B2B employee users.

* **frontend/src/shared/components/AppBootstrap.jsx**
  - Updated `AppBootstrap`'s self-healing synchronization check to import `useB2bStore` and automatically enforce setting `useB2bStore`'s `userRole` to `'business_buyer'` if the user has an active B2B session. This prevents page refreshes from reverting active B2B users back to the customer B2C portal view.

## 2026-06-22 (Session 1 — B2B Employee Company Profile Visibility)

### Updated

* **backend/src/modules/b2bUser/routes/adminDashboard.routes.js**
  - Removed `requireAdmin` restriction from the `GET /company` endpoint to allow B2B Employees (`b2bEmployee` role) to fetch the company profile.

* **backend/src/modules/b2bUser/controllers/profile.controller.js**
  - Updated `getCompanyProfile` to retrieve the company admin's profile and attach admin contact details (name, email, phone) to the returned company object, enabling employee profiles to render administrative info.

* **frontend/src/modules/UserApp/pages/Profile.jsx**
  - Added checks for `isB2BEmployee` and `isB2BUser` to fetch the database-sourced company profile (`dbCompanyProfile`) for employees on tab selection.

## 2026-06-20 (Session 4 — Direct PO Viewer & Timeline Scrollability)

### Updated

* **backend/src/modules/admin/controllers/adminRfq.controller.js**
  - Updated `getAdminPurchaseOrders` to support filtering purchase orders by `rfqId` from query parameters (`req.query.rfqId`).

* **frontend/src/modules/Admin/pages/b2b-enquiries/AdminB2BEnquiries.jsx**
  - Added state for `selectedPo` and helper `handleViewPoForRfq` to fetch specific Purchase Order data.
  - Intercepted the table row click and "Quick Preview" actions click handlers when status is `"Purchase Order Generated"` or `"Completed"`, directly rendering the PO sheet details modal instead of the default quick preview drawer.
  - Rendered a premium A4 contract sheet PO Viewer Modal matching the B2B Admin panel layout, complete with Print/PDF export actions.
  - Added missing `FiPrinter`, `FiTruck`, and `FiFileText` icon imports.

* **frontend/src/modules/Admin/pages/b2b-enquiries/AdminRFQDetail.jsx**
  - Added `max-h-[480px] overflow-y-auto pr-2` to the RFQ Event Timeline list container, ensuring it stays compact and scrollable, matching the discussion chat feed height and preventing layout stretch.
